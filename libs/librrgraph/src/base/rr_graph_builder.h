...
class RRGraphBuilder {
  ...
  public:
    ...
    /** @brief Return a read-only object for rr graph */
    const t_rr_graph_storage& rr_graph() const;

    /* -- VTR 9 API EXTENSIONS FOR INTERPOSER SUPPORT -- */
    
    /**
     * @brief CONSTRUCTION-TIME EDGE QUERY: Get edge information during RR graph construction
     * This is safe to call before edges_read_ is set to true
     */
    struct ConstructionEdgeInfo {
        RRNodeId sink_node;
        short switch_id;
        bool valid;
        ConstructionEdgeInfo() : sink_node(RRNodeId::INVALID()), switch_id(-1), valid(false) {}
        ConstructionEdgeInfo(RRNodeId sink, short sw) : sink_node(sink), switch_id(sw), valid(true) {}
    };
    
    /**
     * @brief Get edge information during construction (SAFE)
     * Returns edge info without requiring edges_read_ = true
     */
    std::vector<ConstructionEdgeInfo> get_construction_edges(RRNodeId src_node);
    
    /**
     * @brief SAFE EDGE TRANSFER: Transfer edges between nodes during construction
     * Handles edge deletion and creation while maintaining consistency
     */
    void safe_transfer_edge(RRNodeId src_node, RRNodeId old_sink, RRNodeId new_sink);
    
    /**
     * @brief COORDINATE SYNCHRONIZATION: Update node coordinates with index sync
     * Updates coordinates and maintains spatial lookup consistency
     */
    void update_node_coordinates_sync(RRNodeId node_id, short xlow, short ylow, short xhigh, short yhigh);
    
    /**
     * @brief CONSTRUCTION-TIME EDGE DELETION: Remove specific edge during construction
     * Safely removes edges without affecting other edges
     */
    void safe_delete_construction_edge(RRNodeId src_node, RRNodeId sink_node);
    
    /**
     * @brief BUILD CONSTRUCTION FANIN MAP: Create reverse edge mapping during construction
     * Builds fanin relationships without accessing finalized edge structures
     */
    std::map<RRNodeId, std::vector<RRNodeId>> build_construction_fanin_map();
    
    /**
     * @brief BATCH EDGE OPERATIONS: Process multiple edge changes atomically
     * Ensures consistency when making multiple edge modifications
     */
    struct EdgeOperation {
        enum Type { DELETE_EDGE, CREATE_EDGE, TRANSFER_EDGE };
        Type operation;
        RRNodeId src_node;
        RRNodeId sink_node;
        RRNodeId new_sink_node; // For transfer operations
        short switch_id;
        
        EdgeOperation(Type op, RRNodeId src, RRNodeId sink, short sw = 0) 
            : operation(op), src_node(src), sink_node(sink), new_sink_node(RRNodeId::INVALID()), switch_id(sw) {}
        EdgeOperation(RRNodeId src, RRNodeId old_sink, RRNodeId new_sink, short sw) 
            : operation(TRANSFER_EDGE), src_node(src), sink_node(old_sink), new_sink_node(new_sink), switch_id(sw) {}
    };
    
    void batch_edge_operations(const std::vector<EdgeOperation>& operations);
