...
void check_rr_graph(const RRGraphView& rr_graph,
                    ...
                    bool is_flat) {
    //VTR_LOG("DEBUG: ===== ENTERING check_rr_graph() VALIDATION =====\n");
    //VTR_LOG("DEBUG: Graph stats: %zu nodes, starting validation...\n", rr_graph.num_nodes());
    
    // Add comprehensive validation before starting
    //VTR_LOG("DEBUG: Validating RRGraphView basic properties...\n");
    try {
        size_t num_nodes = rr_graph.num_nodes();
        //VTR_LOG("DEBUG: ✅ rr_graph.num_nodes() = %zu\n", num_nodes);
        
        size_t num_switches = rr_graph.num_rr_switches();
        //VTR_LOG("DEBUG: ✅ rr_graph.num_rr_switches() = %zu\n", num_switches);
        
        // Test a few random node accesses to see if the data is valid
        if (num_nodes > 0) {
            //VTR_LOG("DEBUG: Testing basic node access...\n");
            RRNodeId test_node(0);
            auto test_type = rr_graph.node_type(test_node);
            //VTR_LOG("DEBUG: ✅ Node 0 type: %d\n", (int)test_type);
            
            if (num_nodes > 100) {
                RRNodeId test_node2(100);
                auto test_type2 = rr_graph.node_type(test_node2);
                //VTR_LOG("DEBUG: ✅ Node 100 type: %d\n", (int)test_type2);
            }
            
            if (num_nodes > 1000) {
                RRNodeId test_node3(1000);
                auto test_type3 = rr_graph.node_type(test_node3);
                //VTR_LOG("DEBUG: ✅ Node 1000 type: %d\n", (int)test_type3);
            }
            
            // Test nodes() iterator
            //VTR_LOG("DEBUG: Testing nodes() iterator...\n");
            auto nodes_iter = rr_graph.nodes();
            //VTR_LOG("DEBUG: ✅ nodes() iterator created successfully\n");
        }
        
    } catch (const std::exception& e) {
        //VTR_LOG("DEBUG: ❌ EXCEPTION in basic validation: %s\n", e.what());
        VTR_ASSERT(false);
    } catch (...) {
        //VTR_LOG("DEBUG: ❌ UNKNOWN EXCEPTION in basic validation\n");
        VTR_ASSERT(false);
    }
    ...
    //VTR_LOG("DEBUG: Allocating validation data structures...\n");
    ...
    //VTR_LOG("DEBUG: Data structures allocated successfully\n");

    //VTR_LOG("DEBUG: Starting main validation loop over %zu nodes...\n", rr_graph.num_nodes());
    size_t nodes_processed = 0;
    
    try {
        //VTR_LOG("DEBUG: About to start nodes() iteration...\n");
        
        for (const RRNodeId& rr_node : rr_graph.nodes()) {
            nodes_processed++;
            
            // More frequent progress updates to catch the exact failing node
            if (nodes_processed % 100 == 0) {
                //VTR_LOG("DEBUG: Processed %zu/%zu nodes in validation...\n", nodes_processed, rr_graph.num_nodes());
            }
            
            size_t inode = (size_t)rr_node;
            
            try {
                //VTR_LOG("DEBUG: [Node %zu] Starting validation...\n", inode);
                
                //VTR_LOG("DEBUG: [Node %zu] Calling validate_node()...\n", inode);
                rr_graph.validate_node(rr_node);
                //VTR_LOG("DEBUG: [Node %zu] ✅ validate_node() completed\n", inode);

                /* Ignore any uninitialized rr_graph nodes */
                //VTR_LOG("DEBUG: [Node %zu] Checking if node is initialized...\n", inode);
                if (!rr_graph.node_is_initialized(rr_node)) {
                    //VTR_LOG("DEBUG: [Node %zu] Node not initialized, skipping...\n", inode);
                    continue;
                }
                //VTR_LOG("DEBUG: [Node %zu] ✅ Node is initialized\n", inode);

                // Virtual clock network sink is special, ignore.
                //VTR_LOG("DEBUG: [Node %zu] Checking if virtual clock network root...\n", inode);
                if (rr_graph.is_virtual_clock_network_root(rr_node)) {
                    //VTR_LOG("DEBUG: [Node %zu] Virtual clock network root, skipping...\n", inode);
                    continue;
                }
                //VTR_LOG("DEBUG: [Node %zu] ✅ Not virtual clock network root\n", inode);

                //VTR_LOG("DEBUG: [Node %zu] Getting node type...\n", inode);
                t_rr_type rr_type = rr_graph.node_type(rr_node);
                //VTR_LOG("DEBUG: [Node %zu] ✅ Node type: %d\n", inode, (int)rr_type);
                
                //VTR_LOG("DEBUG: [Node %zu] Getting num_edges...\n", inode);
                int num_edges = rr_graph.num_edges(RRNodeId(inode));
                //VTR_LOG("DEBUG: [Node %zu] ✅ Has %d edges\n", inode, num_edges);

                //VTR_LOG("DEBUG: [Node %zu] Calling check_rr_node()...\n", inode);
                check_rr_node(rr_graph, rr_indexed_data, grid, chan_width, route_type, inode, is_flat);
                //VTR_LOG("DEBUG: [Node %zu] ✅ check_rr_node() completed\n", inode);

                /* Check all the connectivity (edges, etc.) information.                    */
                edges.resize(0);
                edges.reserve(num_edges);

                //VTR_LOG("DEBUG: [Node %zu] Starting edge validation (%d edges)...\n", inode, num_edges);
                for (int iedge = 0; iedge < num_edges; iedge++) {
                    try {
                        //VTR_LOG("DEBUG: [Node %zu] Processing edge %d/%d...\n", inode, iedge, num_edges);
                        
                        //VTR_LOG("DEBUG: [Node %zu] Getting sink node for edge %d...\n", inode, iedge);
                        int to_node = size_t(rr_graph.edge_sink_node(rr_node, iedge));
                        //VTR_LOG("DEBUG: [Node %zu] ✅ Edge %d connects to node %d\n", inode, iedge, to_node);
                        ...
                        //VTR_LOG("DEBUG: [Node %zu] Calling check_rr_edge() for edge %d...\n", inode, iedge);
                        check_rr_edge(rr_graph,
                                      ...
                                      to_node,
                                      is_flat);
                        //VTR_LOG("DEBUG: [Node %zu] ✅ check_rr_edge() completed for edge %d\n", inode, iedge);

                        edges.emplace_back(to_node, iedge);
                        total_edges_to_node[to_node]++;

                        //VTR_LOG("DEBUG: [Node %zu] Getting switch type for edge %d...\n", inode, iedge);
                        auto switch_type = rr_graph.edge_switch(rr_node, iedge);
                        //VTR_LOG("DEBUG: [Node %zu] ✅ Edge %d switch type: %d\n", inode, iedge, (int)switch_type);
                        ...
                        //VTR_LOG("DEBUG: ❌ EXCEPTION in edge %d processing for node %zu: %s\n", iedge, inode, e.what());
                        VTR_ASSERT(false);
                    } catch (...) {
                        //VTR_LOG("DEBUG: ❌ UNKNOWN EXCEPTION in edge %d processing for node %zu\n", iedge, inode);
                        VTR_ASSERT(false);
                    }
                } /* End for all edges of node. */
                
                //VTR_LOG("DEBUG: [Node %zu] ✅ All edges processed successfully\n", inode);
                ...
                //VTR_LOG("DEBUG: [Node %zu] Calling check_unbuffered_edges()...\n", inode);
                check_unbuffered_edges(rr_graph, inode);
                //VTR_LOG("DEBUG: [Node %zu] ✅ check_unbuffered_edges() completed\n", inode);

                //Check that all config/non-config edges are appropriately organized
                //VTR_LOG("DEBUG: [Node %zu] Checking configurable edges organization...\n", inode);
                ...
                //VTR_LOG("DEBUG: [Node %zu] ✅ Edge organization checks completed\n", inode);
                
                //VTR_LOG("DEBUG: [Node %zu] ✅ ALL VALIDATION COMPLETED SUCCESSFULLY\n", inode);
                
            } catch (const std::exception& e) {
                //VTR_LOG("DEBUG: ❌ EXCEPTION during validation of node %zu: %s\n", inode, e.what());
                //VTR_LOG("DEBUG: ❌ SEGFAULT LOCATION IDENTIFIED: Node %zu validation\n", inode);
                VTR_ASSERT(false);
            } catch (...) {
                //VTR_LOG("DEBUG: ❌ UNKNOWN EXCEPTION during validation of node %zu\n", inode);
                //VTR_LOG("DEBUG: ❌ SEGFAULT LOCATION IDENTIFIED: Node %zu validation\n", inode);
                VTR_ASSERT(false);
            }
        ...
        //VTR_LOG("DEBUG: ❌ EXCEPTION in main validation loop after processing %zu nodes: %s\n", nodes_processed, e.what());
        //VTR_LOG("DEBUG: ❌ SEGFAULT LOCATION IDENTIFIED: Main validation loop\n");
        VTR_ASSERT(false);
    } catch (...) {
        //VTR_LOG("DEBUG: ❌ UNKNOWN EXCEPTION in main validation loop after processing %zu nodes\n", nodes_processed);
        //VTR_LOG("DEBUG: ❌ SEGFAULT LOCATION IDENTIFIED: Main validation loop\n");
        VTR_ASSERT(false);
    }

    //VTR_LOG("DEBUG: ✅ Main validation loop completed successfully for all %zu nodes\n", nodes_processed);
    ...
    // edge or not
    if(is_flat) {
        //VTR_LOG("DEBUG: ✅ check_rr_graph() completed successfully (is_flat=true, skipping fanin validation)\n");
        return;
    }

    //VTR_LOG("DEBUG: Starting fanin validation phase...\n");
