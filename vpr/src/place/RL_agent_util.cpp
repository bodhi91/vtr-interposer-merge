...
std::pair<std::unique_ptr<MoveGenerator>, std::unique_ptr<MoveGenerator>> create_move_generators(PlacerState& placer_state,
    ...
    if (!placer_opts.RL_agent_placement) { // RL agent is disabled
        ...
        const auto& all_blocks = placer_state.blk_loc_registry().movable_blocks_per_type();
        num_movable_blocks_per_type.reserve(all_blocks.size());

        std::transform(
            all_blocks.begin(),
            all_blocks.end(),
            std::back_inserter(num_movable_blocks_per_type),
            [](const std::vector<ClusterBlockId>& innerVec) {
                return static_cast<int>(innerVec.size());
        });
