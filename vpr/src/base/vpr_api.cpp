...
void vpr_create_device(t_vpr_setup& vpr_setup, const t_arch& arch) {
    ...
    vpr_setup_interposer_cut_locations(vpr_setup, arch);
#endif
...
void vpr_setup_interposer_cut_locations(const t_vpr_setup& vpr_setup, const t_arch& arch) {
    ...
    // Copy ALL interposer settings from the setup to device context
    device_ctx.pre_pack_only = vpr_setup.InterposerOpts.pre_pack_only;
    device_ctx.percent_wires_cut = vpr_setup.InterposerOpts.percent_wires_cut;
    device_ctx.num_cuts = vpr_setup.InterposerOpts.num_cuts;
    device_ctx.delay_increase = vpr_setup.InterposerOpts.delay_increase;
    device_ctx.placer_cost_constant = vpr_setup.InterposerOpts.placer_cost_constant;
    device_ctx.constant_type = vpr_setup.InterposerOpts.constant_type;
    device_ctx.allow_chanx_interposer_connections = vpr_setup.InterposerOpts.allow_chanx_interposer_connections;
    device_ctx.transfer_interposer_fanins = vpr_setup.InterposerOpts.transfer_interposer_fanins;
    device_ctx.allow_additional_interposer_fanins = vpr_setup.InterposerOpts.allow_additional_interposer_fanins;
    device_ctx.pct_of_interposer_nodes_each_chany_can_drive = vpr_setup.InterposerOpts.pct_of_interposer_nodes_each_chany_can_drive;
    
    // Get interposer settings from the setup
    int num_cuts = vpr_setup.InterposerOpts.num_cuts;
    
    if (num_cuts <= 0) {
        // No cuts specified, nothing to setup for cut locations
        return;
    }
    
    int ny = (int)device_ctx.grid.height() - 2; // Exclude border tiles
    
    // Validate that cuts can be placed without going through blocks
    ...
    VTR_ASSERT(arch.lcm_of_block_heights > 0);
    
    int num_slices = num_cuts + 1;
    
    // Calculate slice height ensuring alignment with LCM of block heights
    // This ensures cutlines don't go through physical blocks
    int slice_height = ((int)(((int)(ny / arch.lcm_of_block_heights)) / num_slices)) * arch.lcm_of_block_heights;
    
    if (slice_height == 0) {
        // Try with minimum slice height
        slice_height = arch.lcm_of_block_heights;
    }
    
    // Store cut locations in device context
    device_ctx.arch_cut_locations.clear();
    device_ctx.arch_cut_locations.reserve(num_cuts);
    
    for (int cut_counter = 0; cut_counter < num_cuts; ++cut_counter) {
        int cut_location = (cut_counter + 1) * slice_height;
        ...
        VTR_ASSERT(cut_location % arch.lcm_of_block_heights == 0);
        device_ctx.arch_cut_locations.push_back(cut_location);
    }
    
    // Log the cut locations for user information
    VTR_LOG("Interposer cuts are located at the following coordinates:\n");
    for (int cut_counter = 0; cut_counter < num_cuts; ++cut_counter) {
        VTR_LOG("\tCut #%d: y=%d\n", cut_counter + 1, device_ctx.arch_cut_locations[cut_counter]);
    }
    
    VTR_LOG("Height of each SLR (Super Logic Region):\n");
    for (int cut_counter = 0; cut_counter <= num_cuts; ++cut_counter) {
        int slr_start = (cut_counter == 0) ? 1 : device_ctx.arch_cut_locations[cut_counter - 1] + 1;
        int slr_end = (cut_counter == num_cuts) ? ny : device_ctx.arch_cut_locations[cut_counter];
        int slr_height = slr_end - slr_start + 1;
        VTR_LOG("\tSLR #%d: height=%d (y=%d to y=%d)\n", cut_counter + 1, slr_height, slr_start, slr_end);
    }
}
#endif
