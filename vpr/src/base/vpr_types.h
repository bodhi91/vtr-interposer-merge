...
///@brief Interposer Architecture Options
struct t_interposer_opts {
    bool pre_pack_only = false;                                           ///<Run pre-packing only for interposer experiments
    int percent_wires_cut = 0;                                           ///<Percentage of wires to cut at interposer interfaces (0-100)
    int num_cuts = 0;                                                    ///<Number of horizontal cuts to make for interposer boundaries
    int delay_increase = 0;                                              ///<Delay increase factor for wires crossing interposer boundaries
    float placer_cost_constant = 1.0;                                   ///<Cost constant for placer when considering interposer penalties
    int constant_type = 0;                                               ///<Type of constant to use in interposer cost calculations
    bool allow_chanx_interposer_connections = false;                    ///<Allow horizontal channel connections across interposer boundaries
    bool transfer_interposer_fanins = false;                            ///<Transfer fanins across interposer boundaries
    bool allow_additional_interposer_fanins = false;                    ///<Allow additional fanins for interposer nodes
    int pct_of_interposer_nodes_each_chany_can_drive = 0;              ///<Percentage of interposer nodes each channel can drive
    bool allow_interposer_opin_to_wire_fanouts = false;                 ///<Allow output pin to wire fanouts across interposer
    bool allow_interposer_wire_to_ipin_fanins = false;                  ///<Allow wire to input pin fanins across interposer
};
#endif
...
#ifdef INTERPOSER_BASED_ARCHITECTURE
    t_interposer_opts InterposerOpts; ///<Interposer architecture options
#endif
