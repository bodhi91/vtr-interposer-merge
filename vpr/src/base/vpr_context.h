...
struct DeviceContext : public Context {
    ...
    /// Whether to run pre-packing only for interposer experiments
    bool pre_pack_only = false;
    
    /// Percentage of wires to cut at interposer interfaces (0-100)
    int percent_wires_cut = 0;
    
    /// Number of horizontal cuts to make for interposer boundaries
    int num_cuts = 0;
    
    /// Delay increase factor for wires crossing interposer boundaries
    int delay_increase = 1;
    
    /// Placement cost constant for interposer-aware placement
    float placer_cost_constant = 1.0f;
    
    /// Type of constant to use in placement cost calculations
    int constant_type = 0;
    
    /// Mapping of edges with increased delay due to interposer crossings
    std::vector<int> increased_delay_edge_map;
    
    /// Allow connections between CHANX wires and interposer nodes
    bool allow_chanx_interposer_connections = false;
    
    /// Transfer fanins from cut wires to interposer nodes
    bool transfer_interposer_fanins = false;
    
    /// Allow additional fanins beyond the transferred ones
    bool allow_additional_interposer_fanins = false;
    
    /// Percentage of interposer nodes each CHANY wire can drive
    int pct_of_interposer_nodes_each_chany_can_drive = 100;
    
    /// Transfer fanouts from cut wires to interposer nodes
    bool transfer_interposer_fanouts = false;
    
    /// Allow additional fanouts beyond the transferred ones
    bool allow_additional_interposer_fanouts = false;
    
    /// Percentage of CHANY wires an interposer node can drive
    int pct_of_chany_wires_an_interposer_node_can_drive = 100;
    
    /// Y-coordinates of the interposer cut locations
    std::vector<int> arch_cut_locations;
    
    /// Allow bidirectional interposer wires
