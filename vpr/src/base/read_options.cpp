...
argparse::ArgumentParser create_arg_parser(const std::string& prog_name, t_options& args) {
    ...
    auto& interposer_grp = parser.add_argument_group("interposer architecture options");

    interposer_grp.add_argument<bool, ParseOnOff>(args.pre_pack_only, "--pre_pack_only")
        .help("Run pre-packing only for interposer experiments (skip placement and routing)")
        .default_value("off")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument(args.percent_wires_cut, "--percent_wires_cut")
        .help("Percentage of wires to cut at interposer interfaces (0-100)")
        .default_value("0")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument(args.num_cuts, "--num_cuts")
        .help("Number of horizontal cuts to make for interposer boundaries")
        .default_value("0")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument(args.delay_increase, "--delay_increase")
        .help("Delay increase factor for wires crossing interposer boundaries")
        .default_value("1")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument(args.placer_cost_constant, "--placer_cost_constant")
        .help("Placement cost constant for interposer-aware placement")
        .default_value("1.0")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument(args.constant_type, "--constant_type")
        .help("Type of constant to use in placement cost calculations")
        .default_value("0")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument<bool, ParseOnOff>(args.allow_chanx_interposer_connections, "--allow_chanx_interposer_connections")
        .help("Allow connections between CHANX wires and interposer nodes")
        .default_value("off")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument<bool, ParseOnOff>(args.transfer_interposer_fanins, "--transfer_interposer_fanins")
        .help("Transfer fanins from cut wires to interposer nodes")
        .default_value("off")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument<bool, ParseOnOff>(args.allow_additional_interposer_fanins, "--allow_additional_interposer_fanins")
        .help("Allow additional fanins beyond the transferred ones")
        .default_value("off")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument(args.pct_of_interposer_nodes_each_chany_can_drive, "--pct_of_interposer_nodes_each_chany_can_drive")
        .help("Percentage of interposer nodes each CHANY wire can drive")
        .default_value("100")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument<bool, ParseOnOff>(args.transfer_interposer_fanouts, "--transfer_interposer_fanouts")
        .help("Transfer fanouts from cut wires to interposer nodes")
        .default_value("off")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument<bool, ParseOnOff>(args.allow_additional_interposer_fanouts, "--allow_additional_interposer_fanouts")
        .help("Allow additional fanouts beyond the transferred ones")
        .default_value("off")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument(args.pct_of_chany_wires_an_interposer_node_can_drive, "--pct_of_chany_wires_an_interposer_node_can_drive")
        .help("Percentage of CHANY wires an interposer node can drive")
        .default_value("100")
        .show_in(argparse::ShowIn::HELP_ONLY);

    interposer_grp.add_argument<bool, ParseOnOff>(args.allow_bidir_interposer_wires, "--allow_bidir_interposer_wires")
        .help("Allow bidirectional interposer wires")
        .default_value("off")
        .show_in(argparse::ShowIn::HELP_ONLY);
#endif
